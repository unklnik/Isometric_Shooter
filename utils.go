...
// MARK: ISO
func qIBimREC(b IB) z.Rectangle {
	x := b.p[1].X
	y := b.p[2].Y
	w := b.p[3].X - b.p[1].X
	y -= w / 2
	return z.NewRectangle(x, y, w, w)
}
func qZINDEXIsoGrid(ig IG) IG {
	sort.Slice(ig.b, func(i, j int) bool { return ig.b[i].zi > ig.b[j].zi })
	return ig
}

// MARK: SLICES
func REMANIMOFF(a []ANIM) []ANIM {
	var a2 []ANIM
	for i := range a {
		if !a[i].off {
			a2 = append(a2, a[i])
...
// MARK: COLLISIONS
func cIGPOINT(ig IG, p z.Vector2) int {
	num := BLANKNUM
	for i := range ig.b {
		if cTRIPOINT(ig.b[i].t[0], p) || cTRIPOINT(ig.b[i].t[1], p) {
			num = i
			break
		}
	}
...
func cIBPOINT(b IB, p z.Vector2) bool {
	collides := false
	if cTRIPOINT(b.t[0], p) || cTRIPOINT(b.t[1], p) {
		collides = true
	}
	return collides
}
func cIGPOINTdrawIB(ig IG, p z.Vector2, c z.Color) {
	num := cIGPOINT(ig, p)
	if num != BLANKNUM {
		dISOrecSolidIB(ig.b[num], 1, c)
	}
}
func cTRIPOINT(t TRI, p z.Vector2) bool {
	return z.CheckCollisionPointTriangle(p, t.v1, t.v2, t.v3)
}
