...
func dAnimRecLoopColorPART(anm ANIM, numFramesFromStart, fps int, r z.Rectangle, c z.Color) ANIM {
	dIMcolor(anm.ims[anm.dFrame], r, c)
	anm = dAnimFramePART(anm, numFramesFromStart, fps)
	return anm
}
...
func dAnimRecLoopFlipColorPART(anm ANIM, numFramesFromStart, fps int, r z.Rectangle, c z.Color) ANIM {
	dIMflipColor(anm.ims[anm.dFrame], r, c)
	anm = dAnimFramePART(anm, numFramesFromStart, fps)
	return anm
}
...
// MARK: ANIM MAKE
func mAnimSheetIM(im IM, x, y, siz, spc float32, numW, numH, fps int) ANIM {
	ims := mIMSheetIM(im, x, y, siz, spc, numW, numH)
	return mAnimImSheet(ims, fps)
}
...
func dIMroFlip(im IM, r z.Rectangle, angl float32) {
	im.r.Width = -im.r.Width
	z.DrawTexturePro(im.tex, im.r, DREC(r), qORIGIN(r), angl, z.White)
}
func dIMroFlipColor(im IM, r z.Rectangle, angl float32, c z.Color) {
	im.r.Width = -im.r.Width
	z.DrawTexturePro(im.tex, im.r, DREC(r), qORIGIN(r), angl, c)
}
func dIMroFlipColorSHADOW(im IM, r z.Rectangle, angl, offsetShadow float32, c, cShadow z.Color) {
	im.r.Width = -im.r.Width
...
func mIMSheetIM(im IM, x, y, siz, spc float32, numW, numH int) []IM {
	var ims []IM
	a := numW * numH
	im2 := IM{}
	im2.tex = im.tex
	...
	c := 0
	for a > 0 {
		im2.r = z.NewRectangle(x, y, siz, siz)
		ims = append(ims, im2)
		x += siz + spc
		c++
		a--
